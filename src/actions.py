...
def _toggle_bluetooth(os_name: str, payload: str) -> tuple[bool, str, str]:
    ...
    if os_name == "windows":
        action_ps = "Enable" if want in ("on", "") else "Disable"
        script = (f'$bt = [Windows.Devices.Radios.Radio,Windows.System.Devices,ContentType=WindowsRuntime];'
                  f'$r = $bt::RequestAccessAsync().GetAwaiter().GetResult();'
                  f'($bt::GetRadiosAsync().GetAwaiter().GetResult()'
                  f' | Where {{$_.Kind -eq "Bluetooth"}})'
                  f' | ForEach {{$_.SetStateAsync("{action_ps}").GetAwaiter().GetResult()}}')
        return _sh(["powershell", "-Command", script])
    return False, "", "Unsupported OS"
